...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(app: UIApplication, open url: URL, options: [String : AnyObject]) -> Bool {
        ...
        let now:Int = Int(Date().timeIntervalSince1970)
            ...
            let ext = path.substring(with: path.index(path.endIndex, offsetBy: -3)..<path.index(path.endIndex, offsetBy: 0))
                ...
                if !errorAtImageImport {
                    ...
                    if let polygons = svgRoot["path"].all {
                        for polygon in polygons {
                            ...
                            let path = svgPath.replacingOccurrences(of: ",", with: ";")
